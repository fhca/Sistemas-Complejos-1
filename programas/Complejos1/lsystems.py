...
class Lsystem(Turtle):
    ...
    def __init__(self, lsys="", ops=""):
        ...
        if lsys == "":
            lsys = {"axiom": "0", "rules": {"1": "11", "0": "1[0]0"},
                    "langle": 45, "rangle": 45, "startpos": (0,-300), "startheading": 90}
        # if ops == "":
        #     ops = {"1":self.segment, "0": self.leaf_segment, "[": self.push_left, "]": self.pop_right}
        if ops == "":
            ops = {"1": "segment", "0": "leaf_segment", "[": "push_left", "]": "pop_right",
                   "(": "push_pos_angle", ")": "pop_pos_angle"}
    ...
    def _iterate(self, str):
        ...
        return "".join(rules.get(x, x) for x in str) # si no encuentra el símbolo, lo deja ahí
...
class GatherLsystems:
    ...
    def draw(self):
        l = [(t, t.draw()) for t in self.done.keys()]
        done = False
        while not done:
            for turtle, turtle_draw in l:
                try:
                    next(turtle_draw)
...
"branch large"
Lsystem({"axiom": "0", "rules": {"1": "11", "0": "1[0]0"}, "langle": 3, "rangle": 7,
             "startpos": (0,-300), "scale":8})

"branch small"
Lsystem({"axiom": "0", "rules": {"1": "11", "0": "1[0]0"}, "langle": 15, "rangle": 8,
             "startpos": (-100,-300), "depth":6})

"square koch"
Lsystem({"axiom": "F", "rules": {"F": "F+F-F-F+F"}, "angle": 90, "scale": 1.3, "color": "red"})

"tepee"
Lsystem({"axiom": "G", "rules": {"G": "GFX(+G)(-G)", "X": "X(-FFF)(+FFF)FX"}, "startpos" : (-200, -280)})

"sierp"
Lsystem({"axiom": "A", "rules": {"A": "C+B-A-B+", "B": "C-A+B+A-"}, "angle": 60, "startpos": (100,-300),
                 "scale": 1.3, "depth": 8, "color": "brown"},
                {"A": "segment", "B": "segment"})

"plant"
Lsystem({"axiom": "X", "rules": {"X": "F-((X)+X)+F(+FX)-X", "F": "FF"}, "angle": 20, "startpos": (250, -100),
         "scale": 3, "depth": 6, "color": "darkblue"})

"dragon"
Lsystem({"axiom": "X", "rules": {"X": "X+YF+", "Y": "-FX-Y"}, "angle": 90, "startpos": (-250, -300),
         "scale": 3, "depth": 8, "color": "darkblue"})
